...
- An AxonFlow-issued license key may cause the software to apply higher ceilings or enable additional tier-gated capabilities. **That key is not necessarily a paid one:** AxonFlow issues free, time-limited Evaluation keys that raise several of these ceilings. `EvaluationLimits` (`platform/agent/license/tier_limits.go`) sets 50 tenant policies, 75 human principals and 25 service principals against Community's 25 and 5, and 14-day audit retention, and enables policy simulation and evidence export. It does NOT raise organization policies: that ceiling is 0 on Community and on Evaluation alike, because authoring at the organization tier is an Enterprise capability.
...
|---|---:|---|
| Tenant-tier policies | 20 | `platform/agent/static_policy_repository.go` (`MaxTenantPoliciesCommunity`); reported by `platform/agent/license/tier_limits.go` (`CommunityLimits.TenantPolicies`) |
| Organization-tier policies | 0 | `platform/orchestrator/policy_api_service.go` (`validateTierForCreate`, `ImportPolicies` and `TypedAuthoringRouteHandler.handlePublish` all call `admitOrgRootPolicies`, the orchestrator's one call site of the admission decision - `AdmitAll` since #3973 made a document's policies one all-or-nothing batch, pinned by the call-site census; the limit is `OrgPolicies` in `platform/agent/license/tier_limits.go`, 0 on Community and on Evaluation, refused with `ERR_TIER_LIMIT_ORG_ROOT_POLICY`); reported by `platform/orchestrator/policy_tier_support.go` (`DefaultLicenseChecker.OrgPolicyLimit`, which returns a literal `0`) |
...
| Custom policy connectors | none | Retired in v11 with the MCP dynamic-policy plane (PRD v11 §1.2): no edition caps it and nothing enforces it. `CustomPolicyConnectors` in `platform/agent/license/tier_limits.go` stays on the licence wire, deprecated |
| Audit retention (days), `audit_logs` only | 3 | `platform/agent/license/tier_limits.go` (`CommunityLimits.AuditRetentionDays`), applied to the `audit_logs` table by `platform/orchestrator/audit_cleanup.go`. **Six other audit tables are not governed by this number:** they run off `audit_retention_config` with a five-year fallback (`defaultRetentionDaysFallback = 1825`) and are dry-run unless `AXONFLOW_AUDIT_RETENTION_ENFORCE` is set. Do not read 3 days as the retention of your whole audit estate |
| LLM providers | 2 | `platform/agent/license/tier_limits.go` (`CommunityLimits.MaxLLMProviders`) |
| Execution history retained | 50 | `platform/agent/license/tier_limits.go` (`CommunityLimits.MaxExecutionHistory`) |
| Concurrent executions | 5 | `platform/agent/license/tier_limits.go` (`CommunityLimits.MaxConcurrentExec`) |
...
| Human-in-the-loop step approvals | `platform/agent/license/tier_limits.go` (`CommunityLimits.HITLApprovalEnabled = false`) |
