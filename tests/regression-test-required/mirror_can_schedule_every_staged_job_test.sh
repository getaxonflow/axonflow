...
# A JOB WITH NO `runs-on` IS NOT A FLEET JOB (#3919). 76 jobs in this tree
# declare none - reusable-workflow callers, which carry `uses:` at job level
# and have no steps of their own to run anywhere. A cold-cache re-audit found
# that making labels_for(None) return {'self-hosted'} reclassified every one of
# them as fleet and NO suite noticed, including the guard that exists to stop
# fleet jobs mutating host-global state: it inspects steps, and these have
# none, so a silent reclassification of 76 jobs produced not one complaint.
# The gap was in coverage, not in the code - the code is right, and now it is
# also held to being right.
assert is_fleet(None) is False,              "a job with no runs-on must not read as FLEET"
assert schedulable_on_mirror(None) is True,  "a job with no runs-on needs no fleet, so the mirror can schedule it"
...
# rule 1 says yes to. The example this rule was written from:
# runner-fleet-health.yml ran on ubuntu-latest - it has to, so it can still
# report when the fleet is down - and asked `/repos/{repo}/actions/runners`.
# On the mirror that is `gh: Resource not accessible by integration (HTTP
# 403)`, every 30 minutes, on a PUBLIC repository, blocking nothing. 47 of 47
# runs of it on the mirror in the 24 hours to 2026-09-08T09:03Z failed - every
# one (run 34203530117).
#
# #3912 rebuilt that workflow and it no longer reads the inventory anywhere -
# it probes the fleet's labels instead - so it is no longer an instance of
# THIS rule, only of rule 1. The rule stays: reading the inventory is a way to
# depend on a fleet the mirror has not got whether or not any current file
# does it, and a guard that only exists while an example does is a guard that
# retires itself the moment it works.
