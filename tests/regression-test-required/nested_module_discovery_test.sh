...
# THE SCOPE IS AN EXCLUSION RULE, NOT A LIST OF ROOTS (#3940).
#
# This walked ("ee", "examples") until #3940. That list was written the day the
# guard was, and every module outside it was invisible: `runtime-e2e/` held a
# module that ran in no job and built nowhere, and nothing here could say so
# because nothing here looked. A list is a census bounded by its writing date;
# only a rule is about the class, and the tell is that the guard's own header
# said "every separate Go module" while its scope took two names.
#
# So the scope is now every top-level entry MINUS the ones named below, each
# with its reason, plus "." for a go.mod at the repository root.
EXCLUDED_TOPS = {
    ".git",            # not source
    ".github",         # workflows; no Go module lives here
    "node_modules",    # vendored JS, never tracked Go
    "coverage",        # generated
}
def modules():
    out = []
    tops = sorted(
        ...
        if p.is_dir() and p.name not in EXCLUDED_TOPS and not p.name.startswith(".")
    )
    # "." catches a go.mod at the repository root, which a directory walk over
    # top-level entries misses.
...
# `git ls-files '*go.mod'` is whole-tree discovery and is STRICTLY BETTER than a
# `find` over named roots: it is the tracked tree minus whatever .gitignore
# already removes, so it cannot be one directory short the way a root list can.
# Recognised as the root ".", which prefixes every module path.
GIT_LS_RE = re.compile(r"(?:^|\$\(|[(;|&]\s*|\|\|\s*)\s*git\s+ls-files\s+[^\n]*go\.mod", re.M)
...
mods = modules()
if len(mods) < 128:
    # 128, not 120: the PRE-WIDENING scope (ee/ + examples/) yields 122, so a
    # floor of 120 ADMITTED a revert of the scope rule. R3 F1/F2 (#3981).
    print(f"VACUOUS: only {len(mods)} go.mod files found in the tree (132 on 2026-09-10, when the scope became an "
          f"exclusion rule rather than the roots ee/ and examples/); discovery is not reading the tree")
    sys.exit(3)

# THE POSITIVE CONTROL FOR THE SCOPE RULE ITSELF, and it is DERIVED (R3 F1).
# A count floor alone cannot see a scope revert once the out-of-root TSV rows
# retire: reverting `tops` to ["ee","examples"] AND deleting those rows scored
# 15/0, "scanned 122". So assert MEMBERSHIP too - every top that actually holds
# a go.mod on disk must contribute at least one module to `mods`. That is a
# statement about the CLASS, so it covers a top nobody has thought of yet.
_missing = []
for _p in sorted(root.iterdir()):
    if not _p.is_dir() or _p.name in EXCLUDED_TOPS or _p.name.startswith("."):
        continue
    _on_disk = False
    for _dp, _dn, _fn in os.walk(_p):
        _dn[:] = [d for d in _dn if d not in SKIP]
        if "go.mod" in _fn:
            _on_disk = True
            break
    if _on_disk and not any(m == _p.name or m.startswith(_p.name + "/") for m in mods):
        _missing.append(_p.name)
if _missing:
    print(f"VACUOUS: these top-level directories hold a go.mod on disk but contributed NO module to the "
          f"scan: {', '.join(_missing)}. The scope rule has stopped reading the tree.")
    sys.exit(3)

def derived_lanes_for(mod):
    # "." is the whole tree (a `git ls-files` lane) and prefixes every module.
    return [(j, roots, reads, pr) for (j, roots, reads, pr) in derived
            if any(r == "." or mod == r or mod.startswith(r.rstrip("/") + "/") for r in roots)]
...
for mod, (disp, reason, n) in rows.items():
    if mod not in mods:
        problems.append(f"{TSV_NAME}:{n}: row for {mod}, which has no go.mod anywhere in the tree; delete the row")

if not derived:
    problems.append("no job in test.yml discovers go.mod files at all")
...
print(f"scanned {len(mods)} go.mod files across the tree, {len(rows)} rows, "
...
case $RC in
  0) ok "every go.mod in the tree is discovered by a lane that reads the dispositions file, or is a reasoned row" ;;
...
SKIP = {"node_modules", "testdata", "vendor", ".git"}
# THE FIXTURE MIRRORS THE GUARD'S OWN SCOPE RULE, and had the same defect the
# guard had: it seeded ("ee", "examples") only, so every row for a module
# outside those two roots read as STALE inside the fixture and every control
# reported a defect that exists nowhere. A control harness that models a
# narrower tree than the thing it controls tests a different program.
EXCLUDED_TOPS = {".git", ".github", "node_modules", "coverage"}
tops = sorted(p.name for p in src.iterdir()
              if p.is_dir() and p.name not in EXCLUDED_TOPS and not p.name.startswith("."))
if (src / "go.mod").is_file():
    (dst / "go.mod").write_text("module x\n")
for top in tops:
...
# THE WIDENED SCOPE'S OWN CONTROLS (R3 F1, #3981). Every control above plants
# under ee/ or examples/ - roots already in scope BEFORE the scope became an
# exclusion rule - so none of them fails when the rule is reverted. These two
# do: the first under a root the old list did not name, the second under a top
# that does not exist yet, which is the case a root list can never cover.
seed; mkdir -p "$TMP/t/runtime-e2e/plantmod" && printf 'module plant\n' > "$TMP/t/runtime-e2e/plantmod/go.mod"
expect 1 "control: a NEW runtime-e2e/ module with no row is caught (the scope is a rule, not the old root list)" "runtime-e2e/plantmod: runs in NO job"
seed; mkdir -p "$TMP/t/plantopdir/mod" && printf 'module plant\n' > "$TMP/t/plantopdir/mod/go.mod"
expect 1 "control: a module under a TOP-LEVEL directory nobody anticipated is caught" "plantopdir/mod: runs in NO job"
...
expect 1 "control: a row whose module has no go.mod is stale" "has no go.mod anywhere in the tree"

# 6. A covered-by row naming a job that never enters the module. The module
#    and its row are planted: no real covered-by row has a single `cd` a
#    one-point mutation could remove (#4041 moved the customer portal, the
#    one that did, into the discovering lane).
seed; mkdir -p "$TMP/t/ee/platform/plantmod" && printf 'module plant\n' > "$TMP/t/ee/platform/plantmod/go.mod"
printf 'ee/platform/plantmod\tcovered-by:unit-tests-connectors\tplanted by control 6\n' >> "$T_TSV"
expect 1 "control: a covered-by row naming a job with no \`cd\` into the module is caught" "no code line in that job does \`cd ee/platform/plantmod\`"
...
#    Planted, as in 6, and paired with the same job running `go test` there,
#    which must pass: the defect is the missing test, not the plant.
plant_cd_job() {   # plant_cd_job <command after the cd>
  seed; mkdir -p "$TMP/t/ee/platform/plantmod" && printf 'module plant\n' > "$TMP/t/ee/platform/plantmod/go.mod"
  printf 'ee/platform/plantmod\tcovered-by:plant-cd-job\tplanted by control 9\n' >> "$T_TSV"
  python3 - "$T_WF" "$1" <<'PY'
import sys, pathlib
p = pathlib.Path(sys.argv[1]); t = p.read_text()
plant = f"""
...
plant_cd_job "go test -tags enterprise ./..."
expect 0 "control: the same planted job running go test in the module is clean"
