...
# The controls below plant a STALE value over the CURRENT one, so they must know
# what the tree currently says. They used to hardcode it, which meant that the
# moment a release moved the version the sed matched nothing, no defect was
# planted, and the control reported the CHECKER as blind rather than failing --
# measured on the v11.0.0 prep, where three controls went quiet at once. Derive
# the values from the same sources the stamper reads, and refuse to run if a
# derivation comes back empty: a control that plants nothing proves nothing.
PLATFORM_VER="$(tr -d '[:space:]' < "$REPO_ROOT/VERSION")"
SDK_VER="$(grep -A6 '^var recommendedVersion' "$REPO_ROOT/platform/shared/sdkcompat/sdkcompat.go" | grep -m1 '"go":' | grep -oE '[0-9]+\.[0-9]+\.[0-9]+')"
PY_VER="$(grep -A6 '^var recommendedVersion' "$REPO_ROOT/platform/shared/sdkcompat/sdkcompat.go" | grep -m1 '"python":' | grep -oE '[0-9]+\.[0-9]+\.[0-9]+')"
for pair in "PLATFORM_VER:$PLATFORM_VER" "SDK_VER:$SDK_VER" "PY_VER:$PY_VER"; do
  if [ -z "${pair#*:}" ]; then
    echo "FATAL: ${pair%%:*} derived empty - the controls below would plant nothing" >&2
    exit 1
  fi
done
echo "controls plant against: platform $PLATFORM_VER, sdk $SDK_VER, python $PY_VER"

# 1. The 24-page cluster, reintroduced on one page.
control "a stale **Platform Version:** stamp" \
        docs/guides/proxy-mode.md \
        's/\*\*Platform Version:\*\* '"$PLATFORM_VER"'/**Platform Version:** 9.14.0/' \
        "Platform Version 9.14.0" yes
# 2. The SDK half of the same header.
control "a stale **SDK Version:** stamp" \
        docs/guides/proxy-mode.md \
        's/\*\*SDK Version:\*\* '"$SDK_VER"'/**SDK Version:** 9.0.0/' \
        "SDK 9.0.0" yes
# 3. The tutorial pin a new user copies.
control "a stale pip install pin" \
        docs/tutorials/README.md \
        's/axonflow=='"$SDK_VER"'/axonflow==9.0.0/' \
        "axonflow==9.0.0" yes
# 4. A freshness note that has gone stale.
control "a stale 'current release' freshness note" \
        technical-docs/ARCHITECTURE.md \
        's/current release '"$PLATFORM_VER"'/current release 9.19.0/i' \
...
control "a version stamp that dodges the canonical label" \
        docs/guides/proxy-mode.md \
        's/\*\*Platform Version:\*\* '"$PLATFORM_VER"'/**Platform:** 9.14.0/' \
        "UNCANONICAL" no
...
control "a drifted OpenAPI info.version" \
        ...
        's/^  version: '"$PLATFORM_VER"'$/  version: 2.1.0/m' \
...
seed_copy
perl -pi -e 's/"python":     "'"$PY_VER"'"/"python":     "0.0.0"/' \
