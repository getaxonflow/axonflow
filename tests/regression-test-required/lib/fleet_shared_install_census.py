...
from runs_on_labels import is_fleet  # noqa: E402  (same directory)

# `is_fleet` used to be defined here as `isinstance(runs_on, list) and
# "self-hosted" in runs_on`. That stopped seeing twelve jobs the moment the
# synced workflows moved to a repository-conditional `runs-on`, because the
# value became an expression STRING - and a census that stops seeing the fleet
# reports a confident zero. It now resolves through the shared module, which
# both this census and the mirror control use, so they cannot drift apart.
