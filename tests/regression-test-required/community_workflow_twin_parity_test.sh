...
#
#   1a. test.yml's jobs end with the main-red reporter (#4041), and a job with
#      no checkout of its own gets a sparse one of the action just before it.
#      The reporter acts only in this repository (its rule lives in the
#      action's script) and tests nothing, so those two steps - exactly those,
#      recognised by shape and checked on a planted job - are dropped from
#      test.yml's side before the comparison.
...
# enterprise-only (repository-gates.yml's run-regression-suite gates on github.repository);
...
REPLAY_EXEMPT=$'race-detector :: runs `go test -race` over platform/orchestrator and shared/policy - both have untagged builds that test.yml\'s unit-test jobs execute on every pull request and whose community-build compile the simulation vets; replaying a multi-minute race run over already-proven packages is cost without evidence
...
REPORTER = "./.github/actions/report-main-red"

def drop_reporter(job):
    """test.yml's trailing main-red reporter (#4041) and the sparse checkout
    that resolves it, which test nothing; no other step is ever dropped."""
    steps = list(job.get("steps") or [])
    last = steps[-1] if steps else None
    if isinstance(last, dict) and last.get("uses") == REPORTER and str(last.get("if")) == "always()":
        steps = steps[:-1]
        prev = steps[-1] if steps else None
        if (isinstance(prev, dict) and str(prev.get("uses", "")).startswith("actions/checkout")
                and (prev.get("with") or {}).get("sparse-checkout") == ".github/actions/report-main-red"):
            steps = steps[:-1]
...
_plant = {"steps": [{"run": "x"},
                    {"uses": "actions/checkout@v6", "with": {"sparse-checkout": ".github/actions/report-main-red"}},
                    {"if": "always()", "uses": REPORTER}]}
_other = {"steps": [{"run": "x"}, {"if": "always()", "uses": "actions/upload-artifact@v4"}]}
if drop_reporter(_plant)["steps"] == [{"run": "x"}] and drop_reporter(_other)["steps"] == _other["steps"]:
    ok("control: the reporter and its sparse checkout are dropped from test.yml's side, and any other trailing step is not")
else:
    bad("control: drop_reporter drops the wrong steps")
...
for job_id in shared:
    ...
    if not e_runs and not c_runs:
        ...
        continue
    compared += 1
    e_shape = step_shape(drop_reporter(ent[job_id] or {}))
