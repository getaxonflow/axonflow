...
#   G  detector success, NO outputs map at all                 -> exit 1
#   H  detector success, outputs present but all empty         -> exit 1
...
run_case "G: detector succeeded but declared NO outputs -> RED (wiring, not a path miss)" \
  '{"detect-changes":{"result":"success","outputs":{}},"compile-java-examples":'"$skipped"'}' 1 "no output carrying a value" "::notice::"

# H is the case G alone would have MISSED, and it is the one that matters more.
# A renamed step id or a misspelled output key - the two causes the branch's own
# comment names - do not remove the key from `needs.<job>.outputs`; they leave it
# carrying the EMPTY STRING. R3 round 2 on #3803 could not settle whether GitHub
# omits an empty-valued output, and drove `{"k": ""}` through the round-2 head:
# it was ACQUITTED. So the branch was correct about the defect and wrong about
# its shape. The condition now tests for no output carrying a VALUE, which covers
# both the absent map and the empty one.
run_case "H: detector succeeded, outputs present but ALL EMPTY -> RED (wiring, not a path miss)" \
  '{"detect-changes":{"result":"success","outputs":{"java-examples":""}},"compile-java-examples":'"$skipped"'}' 1 "no output carrying a value" "::notice::"
