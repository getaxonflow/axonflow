...
# The tests hygiene scan (repository-gates.yml) scans all of tests/ for `[a-z0-9-]+\.elb\.amazonaws\.com`
...
# ── run one case ─────────────────────────────────────────────────────────────
# run_case <label> <want_rc> <want_edition|-> <OVERRIDE_URL> <OVERRIDE_ENV> <CHOICE_URL> <EDITION_INPUT> <DS_PROBE>
run_case() {
  local label="$1" want_rc="$2" want_ed="$3"
  local out gh rc got
  gh="$(mktemp)"
  out="$(OVERRIDE_URL="$4" OVERRIDE_ENV="$5" CHOICE_URL="$6" \
         EDITION_INPUT="$7" DS_PROBE="$8" \
...
# msg_case <label> <want-substring> <must-NOT-substring|-> <OVERRIDE_URL> <OVERRIDE_ENV> <CHOICE_URL> <EDITION_INPUT> <DS_PROBE>
#
