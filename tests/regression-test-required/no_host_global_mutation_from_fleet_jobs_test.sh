...
# The mirror carries ~21 of this repository's ~160 workflow files and has zero
# self-hosted runners, so every count below is legitimately small there and the
# anti-vacuity floor would fail a CORRECT tree. Both fleet guards already
# behaved that way on a staged mirror; it was invisible only because the
# mirror's own `run-regression-suite` was itself queued forever for want of a
# runner. Keyed on `ee/`, which the sync excludes by design - never on "the
# census looks small", which is the vacuity hole this floor exists to close.
if [ ! -d ee ]; then
  echo "SKIP: no ee/ - this is the community mirror's tree, which has no self-hosted"
  echo "      fleet to census. This guard asserts a property of the enterprise fleet."
  exit 0
fi
...
import yaml
import sys as _sys; _sys.path.insert(0, 'tests/regression-test-required/lib')
from runs_on_labels import is_fleet
