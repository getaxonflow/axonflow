...
#               append-only CHANGELOG.md. Below the roots, the pinned v10.2.0
#               bytes #3894's upgrade test replays are skipped by path
#               prefix (the finder's PINNED_RELEASE_DIRS).
...
# set near the live population rather than at half of it, because a scanner
# that lost every root but `platform/` would still clear a floor of 1,500 -
# master's R3 MINOR-2. A fixture tree passes 0 and 0.
#
# The citation floor is the live population rounded DOWN to the hundred. It was
# 2,600 until 2026-09-13, when main held 2,608 at 52f6685b2 and 2,611 at
# 8daf9f314, and #3566's removal deleted 90 citations with the files it removes
# (2,610 on its earlier base): 2,518 at its head, 118 of them titled. So the
# floor is 2,500. A scanner reading only `platform/` finds 1,416, still under
# it. The titled floor stays 110.
#
# #4253 lowered it again on 2026-09-14: the tier engine, the segment gate and
# the hold exit went with the files that cited them, 2,518 at its base
# 2ec926f5f and 2,480 at its head, 118 titled both times. So the floor is
# 2,400.
check() {
...
echo "== the real tree =="
OUT="$(check "$REPO_ROOT" "$ADR_DIR" 2400 110)"; RC=$?
echo "$OUT" | sed 's/^/  /'
case $RC in
...
# The pinned v10.2.0 bytes (#3894) are the one exclusion below a root: a
# stale title inside a pin root is not judged, and the same line one
# directory over, or in a live migration, still is. The exclusion is a path
# prefix, not a file type and not a testdata rule.
mk_tree_at() {   # mk_tree_at <name> <repo-relative file> <line-of-text>  -> prints the tree root
  local dir="$TMP/$1"
  mkdir -p "$dir/technical-docs" "$dir/$(dirname "$2")"
  cp -R "$ADR_DIR" "$dir/technical-docs/architecture-decisions"
  printf -- '-- fixture\n%s\n' "$3" > "$dir/$2"
  echo "$dir"
}
PIN_LINE="-- The agent is the only port ($OLD_SEP: Single Entry Point Architecture)."
T="$(mk_tree_at pincore platform/agent/testdata/v10_2_0/migrations/core/019_fixture.sql "$PIN_LINE")"
expect 0 "control: a stale title in the pinned v10.2.0 bytes is not judged (byte-exact released copies, #3894)" "$T"
T="$(mk_tree_at pinee ee/platform/agent/testdata/v10_2_0/migrations/industry/banking/300_fixture.sql "$PIN_LINE")"
expect 0 "control: nor under the ee/ pin root" "$T"
T="$(mk_tree_at pinsibling platform/agent/testdata/v10_2_1/migrations/core/019_fixture.sql "$PIN_LINE")"
expect 1 "control: the same line in a sibling testdata directory is caught (a path prefix, not a testdata rule)" "$T" "names $NEW_SEP"
T="$(mk_tree_at pinlive migrations/core/019_fixture.sql "$PIN_LINE")"
expect 1 "control: the same line in a live migration is caught" "$T" "names $NEW_SEP"
