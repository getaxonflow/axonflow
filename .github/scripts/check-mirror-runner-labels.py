...
2. ASKING ABOUT IT. This rule was written when `runner-fleet-health.yml`
   asked for the fleet INVENTORY, `GET /repos/{repo}/actions/runners`, from a
   job on `ubuntu-latest` - so rule 1 said yes about it, correctly and
   uselessly, while the thing it depended on was unreachable. On the mirror
   that endpoint is a 403 (the mirror's GITHUB_TOKEN cannot read the runner
   list), and even with access the answer would be "zero online", which is the
   mirror working as designed.

   #3912 REBUILT THAT WORKFLOW AND IT NO LONGER READS THE INVENTORY AT ALL: it
   submits a probe job onto the fleet's own labels and watches whether the job
   starts. So today rule 1 would say NO about it, on the probe job, for the
   right reason - and the exclusion stands on that rather than on the 403. The
   example is kept because the SHAPE it illustrates is the point: a dependency
   on a fleet the mirror has not got, invisible to a rule that only reads
   `runs-on`. It had been red on the PUBLIC repository every 30 minutes -
