...
func TestShellCopiesOfRecognisedModesMatchGo(t *testing.T) {
	...
	// failure.
	// `syncedToCommunity` records whether the file reaches the public mirror.
	// `sync-community-repo.yml` excludes `/scripts/*` wholesale and then
	// re-includes `lint-deployment-mode.sh` by name, so on a community checkout
	// the other two are legitimately absent. Without this the test read a file
	// that cannot exist there and turned community CI red for a reason that has
	// nothing to do with the community payload — which is exactly what it did.
	//
	// A missing file is skipped ONLY when the checkout is positively identified
	// as a community one (no `ee/`). Absent-and-`ee/`-present means somebody
	// deleted or moved the script, and that must still fail: "the file is gone"
	// is precisely the drift this test exists to catch, and a bare
	// os.IsNotExist skip would report success for it.
	sources := []struct {
		path              string
		open              string
		close             string
		...
		syncedToCommunity bool   // false = excluded from the community sync filter
	}{
	...
	// the sync, so its absence distinguishes "this tree never had the file"
	// from "the file was removed".
	_, eeErr := os.Stat(filepath.Join(repoRoot, "ee"))
	communityCheckout := os.IsNotExist(eeErr)

	for _, src := range sources {
		t.Run(src.path, func(t *testing.T) {
			data, err := os.ReadFile(filepath.Join(repoRoot, src.path))
			if err != nil {
				if os.IsNotExist(err) && !src.syncedToCommunity && communityCheckout {
					t.Skipf("skipping %s: not present in this checkout (excluded from the community sync filter, "+
						"and `ee/` is absent so this is a community tree); the test runs in full on the enterprise repo", src.path)
				}
