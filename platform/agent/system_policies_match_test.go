...
// This test is only relevant in the enterprise codebase where the
// perf-testing harness is built; it skips when the harness's scenario
// fixture isn't on disk.
//
// SINGLE SOURCE OF TRUTH: scenarios are loaded from the same JSON
// fixture the harness binary embeds at compile time, so a scenario-list
// change automatically flows into the contract check — no duplicated
// lists to keep in sync.
//
// History: this is the test that would have caught the "SELECT * FROM
// products WHERE 1=1" misalignment that surfaced during a prior perf
// run (the harness expected 403 but no seeded pattern caught a bare
// 1=1 without OR, so it landed as Unexpected). The harness scenario
// was updated to use the realistic injection shape (`OR 1=1`) — this
// test guards against the same class of drift on every other scenario.
//
// CONTRACT:
...
func TestPerfHarness_ScenariosAlignWithSeededPatterns(t *testing.T) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			t.Skipf("scenarios fixture not present; enterprise-only test")
		}
