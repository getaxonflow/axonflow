...
// on that tier. The two are never interchangeable, and
// platform/agent/license/admission tests both values on every dimension it
// enforces.
//
// OrgPolicies on Community and Evaluation USED to be this paragraph's live
// example of a zero. It is 20 and 50 now (#3906, #3907), and the remaining
// zeros on this dimension are the three SaaS Plugin tables in tier_support.go -
// which is worth stating because admission.Admit still carries a branch for a
// zero ceiling, and a reader who believed the zero had gone entirely would read
// that branch as dead.

// TierLimits defines the resource limits for a license tier.
type TierLimits struct {
	TenantPolicies int `json:"tenant_policies"`
	// OrgPolicies is the number of CUSTOMER-AUTHORED POLICIES a deployment may
	// hold. RULED 2026-09-08 (#3906, revising #3593 D4): 20 on Community, 50 on
	// Evaluation, unlimited on Enterprise.
	//
	// THE NAME IS OLDER THAN THE MEANING, AND BOTH ARE KEPT DELIBERATELY.
	//
	// The 2026-09-07 ruling set this to 0 / 0 / unlimited on the sentence
	// "organization-root authoring is an Enterprise capability under ee/". That
	// sentence was true in the OLD vocabulary, where a policy had three tiers -
	// system, organization and tenant - and Community authored at the tenant
	// tier. The new model has two signing roots and a scope inside the
	// document, so EVERY customer-authored policy is organization-root, and the
	// same sentence came to mean "Community and Evaluation author nothing".
	// #3906 withdrew it: the edition boundary is which CONSTRUCTS a policy may
	// use, carried by platform/decision/authoring's edition profile, not which
	// root it is written to.
	//
	// The FIELD and the admission dimension it feeds (`org_root_policy`) keep
	// their names. Renaming the dimension would rewrite a value that is in the
	// CHECK constraint of migrations/core/171, in every row already written to
	// principal_admissions, in the metric label domain, and in the wire error
	// code ERR_TIER_LIMIT_ORG_ROOT_POLICY that clients branch on - a migration
	// and a contract break to correct a name that is not even wrong, since
	// every policy this counts genuinely is organization-root. What was wrong
	// was the INFERENCE from the name, and the fix for that is this paragraph.
	OrgPolicies            int  `json:"org_policies"`
	CustomPolicyConnectors int  `json:"custom_policy_connectors"` // DEPRECATED (v11): enforced nowhere since PRD v11 §1.2 retired the MCP dynamic-policy plane; licence validation responses still serialize it
...
var (
	CommunityLimits = TierLimits{
		TenantPolicies: 20,
		// 20, not 0 (#3906/#3907). See the field comment for why the 0 was
		// withdrawn. TenantPolicies stays beside it and is NOT the same budget:
		// it bounds LEGACY tenant-tier rows, enforced by a count on the policy
		// table, while this bounds customer-authored documents through the
		// admission ledger. PRD section 6 folds the two rows into one on paper,
		// and v11 is deliberately the release where both models run so a
		// customer can migrate between them - so during v11 a deployment can
		// hold both budgets, and v12's removal of the legacy path collapses
		// them into this one. Unifying the ENFORCEMENT earlier would need a
		// backfill of existing tenant-tier rows into the ledger, which is a
		// migration this change does not carry (#3907).
		OrgPolicies:            20,
	...
	EvaluationLimits = TierLimits{
		TenantPolicies: 50,
		// 50 (#3906/#3907). This value has now been three things: 5, then 0,
		// then 50, and the middle one is the one worth remembering. The 0 was
		// ruled on 2026-09-07 with the note that "the 5 this carried until then
		// was never backed by an Enterprise-free authoring path" - which was a
		// correct observation about the CODE and the wrong conclusion to draw
		// from it. The absent authoring path was the defect; the limit was the
		// symptom. #3907 builds the path, and #3906 sets the number back above
		// zero to match the ladder the PRD publishes.
		OrgPolicies:            50,
