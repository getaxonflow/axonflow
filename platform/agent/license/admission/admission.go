...
const (
	...
	Node Dimension = "node"
	// OrgRootPolicy is a CUSTOMER-AUTHORED POLICY, keyed by the identity of the
	// thing authored: the policy name on the legacy create and import paths,
	// and the DOCUMENT ID on the typed authoring path (#3907).
	//
	// The name predates the model. In the vocabulary this dimension was written
	// for, "organization-root" distinguished a policy from a tenant-tier one;
	// in the ADR-065 model there is no tenant tier and every customer-authored
	// policy is organization-root, so the word no longer distinguishes anything
	// and the dimension simply counts what a customer wrote. The STRING is
	// deliberately unchanged: it is in migrations/core/171's CHECK constraint,
	// in every row already written, in the metric label domain and in
	// ERR_TIER_LIMIT_ORG_ROOT_POLICY, and renaming it would be a migration and
	// a wire break to correct a name that is not inaccurate.
...
// BatchOutcome is what a Ledger reports for an atomic admit-ALL-under-limit.
//
// THE UNIT OF THE DECISION IS THE BATCH, and that is the whole point of it
// existing beside Outcome. Admitting a document's policies one at a time means
// a document refused at the ceiling leaves the policies before the boundary
// admitted - #3973, measured as 19 of 21 rows written by a publication that was
// refused and never took effect. principal_admissions is append-only, so those
// rows do not come back, and the organization is then at its ceiling with
// nothing published: the refusal tells the author to publish fewer policies and
// following that instruction is impossible.
type BatchOutcome struct {
	// Admitted is how many rows this call wrote.
	Admitted int
	// Existing is how many keys were already present (a replay). They cost
	// nothing: admission is idempotent on the key, which is what lets an author
	// edit a document that already sits at the ceiling.
	Existing int
	// Count is the number of rows on (org, dimension) BEFORE this call.
	Count int
	// Overflow is the FIRST key that would not fit, set only when the batch was
	// refused. It is what lets a refusal name the policy that crossed the
	// boundary rather than saying only "too many", which would leave an author
	// a document to bisect.
	Overflow Key
}
...
type Ledger interface {
	...
	// AdmitAllUnderLimit inserts EVERY new key or NONE of them, under one
	// advisory lock on the shared (org, dimension). Every key must carry the
	// same org and dimension; a mixed batch is an error rather than a
	// best-effort write, because the lock and the count are both per-pair.
	//
	// ALL-OR-NOTHING IS THE PROPERTY, not an optimisation: a batch that does
	// not fit writes nothing at all, so a refused publication costs the caller
	// no capacity. See BatchOutcome.
	AdmitAllUnderLimit(ctx context.Context, keys []Key, limit int, licenceFingerprint string) (BatchOutcome, error)
...
func (a *Admitter) Admit(ctx context.Context, req Request) (Decision, error) {
	...
	if err != nil {
		...
		// honest reason is the ceiling and not "retry in 30 seconds": telling a
		// caller to retry a request that can never succeed is worse than telling
		// it the edition does not carry the thing. R3 round 1, M7.
		//
		// WHICH DIMENSION FIRES THIS HAS CHANGED, and the branch is still live.
		// It was written for org_root_policy, whose ceiling was 0 on both
		// limited self-hosted tiers; #3906 moved that to 20 and 50, so no
		// self-hosted tier carries a zero on it any more. The zeros that remain
		// are the SaaS Plugin tables in tier_support.go, which is why this
		// stays a general branch over `limit == 0` rather than becoming dead
		// code behind a comment naming a dimension that no longer reaches it.
		if limit == 0 {
...
// AdmitAll is the ALL-OR-NOTHING decision for a set of principals that arrive
// together - the policies of one authored document.
//
// # Why it is not a loop over Admit
//
// A loop cannot be all-or-nothing. Each Admit takes its own lock, counts, and
// writes, so a set that does not fit leaves the prefix before the boundary
// admitted while the operation it belonged to is refused. Against an
// append-only ledger those rows never come back: #3973 measured a 21-policy
// publication refused at a ceiling of 20 leaving 19 rows behind and the
// organization unable to publish anything afterwards, including the smaller
// document the refusal instructed it to publish.
//
// # What the caller must guarantee, and what it must not assume
//
// Every principal belongs to ONE (org, dimension), which is what makes a single
// advisory lock sufficient. The ledger refuses a mixed batch rather than
// splitting it.
//
// A REFUSAL NAMES THE PRINCIPAL THAT CROSSED THE BOUNDARY, in the order the
// caller supplied them, so an author is told which policy to remove rather than
// being handed a document to bisect.
func (a *Admitter) AdmitAll(ctx context.Context, dim Dimension, orgID string, principalIDs []string) (Decision, error) {
	if !dim.Valid() {
		return Decision{}, fmt.Errorf("%w: unknown dimension %q", ErrInvalidRequest, string(dim))
	}
	if strings.TrimSpace(orgID) == "" {
		return Decision{}, fmt.Errorf("%w: empty org id", ErrInvalidRequest)
	}
	if dim == Node {
		// The node dimension is a CONCURRENCY question answered by a lease store
		// on every call, with no batch meaning: nodes do not arrive together and
		// a lease is renewed rather than accumulated. Refused rather than
		// silently looped, so a caller cannot believe it got batch semantics.
		return Decision{}, fmt.Errorf("%w: %s is a concurrency dimension and has no batch admission", ErrInvalidRequest, Node)
	}
	keys := make([]Key, 0, len(principalIDs))
	for _, id := range principalIDs {
		p := strings.TrimSpace(id)
		if p == "" {
			return Decision{}, fmt.Errorf("%w: empty principal id", ErrInvalidRequest)
		}
		keys = append(keys, Key{OrgID: orgID, Dimension: dim, PrincipalID: p})
	}
	...
	if len(keys) == 0 {
		// Nothing to admit is not a refusal. A document with no policies is
		// refused by validation long before this, and answering "allowed" here
		// keeps the empty case from looking like a ceiling.
		d.Allowed = true
		d.Source = SourceSeenSet
		return d, nil
	}

	// An unlimited tier returns before any I/O on the request path and records
	// off it, exactly as Admit does; see the note on recordUnlimited.
	if limit < 0 {
		d.Allowed = true
		d.Source = SourceUnlimitedTier
		admissionsTotal.WithLabelValues(string(d.Dimension), d.Edition, string(d.Source)).Add(float64(len(keys)))
		for _, k := range keys {
			rec := d
			rec.PrincipalID = k.PrincipalID
			a.recordUnlimited(rec)
		}
		return d, nil
	}
	...
	a.ledgerCalls.Add(1)
	out, err := bounded(ctx, a.timeout, func(c context.Context) (BatchOutcome, error) {
		return a.ledger.AdmitAllUnderLimit(c, keys, limit, a.fingerprint)
	})
	if err != nil {
		a.noteLedgerError(err)
		// The same honesty rule as Admit: a ceiling of zero can never admit,
		// whatever the store says, so "retry in 30 seconds" would be a lie.
		if limit == 0 {
			return a.refuse(ctx, d, ReasonOverLimit, err), nil
		}
		return a.refuse(ctx, d, ReasonDependencyUnreachable, err), nil
	...
	a.noteLedgerSuccess()
	d.Count = out.Count
	if out.Overflow.PrincipalID != "" {
		// REFUSED, having written nothing. The principal named is the first one
		// that did not fit.
		d.PrincipalID = out.Overflow.PrincipalID
		return a.refuse(ctx, d, ReasonOverLimit, nil), nil
	}
	for _, k := range keys {
		a.seen.Add(k)
	...
	admissionsTotal.WithLabelValues(string(d.Dimension), d.Edition, string(d.Source)).Add(float64(out.Admitted + out.Existing))
	return d, nil
}
...
//
// The seen-set is the dedupe: one write per identity per process, not one per
// request. A NODE is still excluded - a lease is re-taken on the next heartbeat
// and is not a lifetime fact - but org_root_policy is NOT, since #3907. See the
// filter below for why that changed.
func (a *Admitter) recordUnlimited(d Decision) {
	if a.ledger == nil {
		return
	}
	// THE REVISIT CONDITION THAT USED TO BE HERE HAS FIRED, AND THIS IS IT
	// BEING ACTED ON (#3907, #3906).
	//
	// The note said: "REVISIT WHEN org_root_policy stops being 0 on both
	// limited tiers. The exclusion is only free because a downgrade lands on a
	// ceiling of 0, where a recorded row would change nothing - every name is
	// refused either way." #3906 set that ceiling to 20 and 50, so the
	// exclusion stopped being free on the same ruling, and the filter moves
	// WITH the ruled value rather than after it, exactly as the note required.
	//
	// WHAT THE ROW BUYS ON THIS DIMENSION IS NOT WHAT IT BUYS ON THE PRINCIPAL
	// ONES, and the difference is worth stating, because the original exclusion
	// was right about the mechanism and wrong about the consequence.
	//
	// On a principal dimension a recorded row lets an ALREADY-ADMITTED
	// principal keep working after a lapse, because a principal is re-admitted
	// on every request. A policy is admitted once, at creation, and nothing
	// re-admits it - so that argument genuinely does not apply here, and
	// policies already written go on being enforced after a lapse either way.
	//
	// What the row buys instead is the COUNT. AdmitUnderLimit bounds a new
	// admission against the number of rows on (org, dimension). An Enterprise
	// deployment that wrote 500 documents and recorded none of them lapses to
	// Community with a ledger count of ZERO, and may then create 20 MORE on top
	// of the 500 - a ceiling that means nothing on precisely the deployment it
	// exists to bound. Recording makes the post-lapse ceiling describe the
	// deployment rather than the last few minutes of it.
	//
	// The NODE dimension stays excluded, unchanged: a lease is a concurrency
	// fact answered by node_leases on every heartbeat, and Admit routes it
	// there before the ledger is touched, so it has no ledger count to inform.
	if d.Dimension == Node {
		return
	}
