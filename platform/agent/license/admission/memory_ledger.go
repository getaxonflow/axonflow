...
// AdmitAllUnderLimit implements Ledger: every new key or none of them.
func (m *MemoryLedger) AdmitAllUnderLimit(_ context.Context, keys []Key, limit int, _ string) (BatchOutcome, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if err := m.fail(); err != nil {
	...
	if limit < 0 {
		return BatchOutcome{}, errors.New("admission memory ledger: unlimited sentinel reached the ledger")
	}
	if len(keys) == 0 {
	...
	for _, k := range keys {
		if k.OrgID != org || k.Dimension != dim {
			return BatchOutcome{}, errors.New("admission memory ledger: a batch must share one (org, dimension)")
		}
	}
	var out BatchOutcome
	for row := range m.rows {
		if row.OrgID == org && row.Dimension == dim {
			out.Count++
		}
	}
