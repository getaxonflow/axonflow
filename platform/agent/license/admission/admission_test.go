...
func (f *fakeLedger) AdmitAllUnderLimit(_ context.Context, keys []Key, limit int, _ string) (BatchOutcome, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.calls++
	if f.down {
		return BatchOutcome{}, errors.New("ledger down")
	}
	if limit < 0 {
		return BatchOutcome{}, errors.New("unlimited sentinel reached the ledger")
	}
	if len(keys) == 0 {
	...
	for _, k := range keys {
		if k.OrgID != org || k.Dimension != dim {
			return BatchOutcome{}, errors.New("a batch must share one (org, dimension)")
		}
	}
	out := BatchOutcome{Count: f.count(org, dim)}
	room := limit - out.Count
	seen := map[string]bool{}
	var newKeys []Key
	for _, k := range keys {
		if seen[k.PrincipalID] {
			continue
		}
		seen[k.PrincipalID] = true
		if f.rows[k] {
			out.Existing++
			continue
		}
		...
		if len(newKeys) > room {
			return BatchOutcome{Count: out.Count, Existing: out.Existing, Overflow: k}, nil
		}
	}
...
// TestABatchIsAllOrNothing is #3973's in-process evidence, and it exists
// because nothing else exercises the batch at all: every production caller
// today sends a batch of ONE, so the entire all-or-nothing property was
// unasserted and two mutants survived the whole suite.
//
// It runs against BOTH ledgers that implement it. The fake is the counting one
// the rest of this file uses; MemoryLedger is the one other packages wire, and
// it is a SEPARATE implementation of the same rule - a test that drove only the
// fake would say nothing about the ledger the orchestrator's own unit tests
// run over.
func TestABatchIsAllOrNothing(t *testing.T) {
	type ledger interface {
		Ledger
		rows(org string, d Dimension) int
	}
	for _, tc := range []struct {
		name string
		make func() ledger
	}{
		{"fakeLedger", func() ledger { return fakeRows{newFakeLedger()} }},
		{"MemoryLedger", func() ledger { return memRows{NewMemoryLedger()} }},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Run("a batch that does not fit writes NOTHING", func(t *testing.T) {
				l := tc.make()
				a := New(l, WithTierReader(readerFor(license.TierCommunity, LicenceValid)),
					WithLimits(func(license.Tier) license.TierLimits { return license.TierLimits{OrgPolicies: 2} }))
				dec, err := a.AdmitAll(context.Background(), OrgRootPolicy, "org", []string{"p1", "p2", "p3"})
				if err != nil {
					t.Fatal(err)
				}
				if dec.Allowed {
					t.Fatal("three policies were admitted under a ceiling of two")
				}
				if dec.Reason != ReasonOverLimit {
					t.Fatalf("reason=%s, want %s", dec.Reason, ReasonOverLimit)
				}
				// THE MUTANT THIS KILLS: committing the keys that DO fit before
				// reporting the overflow. That is exactly the 19-of-21 defect,
				// and it leaves the caller at their ceiling with nothing
				// published.
				if got := l.rows("org", OrgRootPolicy); got != 0 {
					t.Fatalf("a refused batch wrote %d row(s); the ledger is append-only, so those slots never come "+
						"back and the organization is at its ceiling with nothing published", got)
				}
				// The refusal names the FIRST key that did not fit, in SUPPLIED
				// order - not the last, and not an arbitrary one.
				if dec.PrincipalID != "p3" {
					t.Fatalf("the refusal names %q, want p3: an author told only \"too many\" has a document to bisect", dec.PrincipalID)
				}
			})

			t.Run("a batch that fits writes ALL of it", func(t *testing.T) {
				l := tc.make()
				a := New(l, WithTierReader(readerFor(license.TierCommunity, LicenceValid)),
					WithLimits(func(license.Tier) license.TierLimits { return license.TierLimits{OrgPolicies: 2} }))
				dec, err := a.AdmitAll(context.Background(), OrgRootPolicy, "org", []string{"p1", "p2"})
				if err != nil || !dec.Allowed {
					t.Fatalf("a batch AT the ceiling was refused: dec=%+v err=%v", dec, err)
				}
				if got := l.rows("org", OrgRootPolicy); got != 2 {
					t.Fatalf("wrote %d rows, want 2 - the anti-vacuity for the zero above", got)
				}
			})

			t.Run("duplicate ids in one document spend ONE slot", func(t *testing.T) {
				l := tc.make()
				a := New(l, WithTierReader(readerFor(license.TierCommunity, LicenceValid)),
					WithLimits(func(license.Tier) license.TierLimits { return license.TierLimits{OrgPolicies: 2} }))
				// Three entries, two distinct. Counted naively this is 3 > 2 and
				// would be refused; a document naming the same policy twice must
				// not be charged twice.
				dec, err := a.AdmitAll(context.Background(), OrgRootPolicy, "org", []string{"p1", "p1", "p2"})
				if err != nil || !dec.Allowed {
					t.Fatalf("duplicate ids were charged separately: dec=%+v err=%v", dec, err)
				}
				if got := l.rows("org", OrgRootPolicy); got != 2 {
					t.Fatalf("wrote %d rows for two distinct ids, want 2", got)
				}
			})

			t.Run("a mixed batch is REFUSED, not split", func(t *testing.T) {
				l := tc.make()
				_, err := l.AdmitAllUnderLimit(context.Background(),
					[]Key{{OrgID: "a", Dimension: OrgRootPolicy, PrincipalID: "p"},
						{OrgID: "b", Dimension: OrgRootPolicy, PrincipalID: "q"}}, 5, "")
				if err == nil {
					t.Fatal("a batch spanning two organizations was accepted; the advisory lock is per (org, dimension), " +
						"so splitting it would silently give up the atomicity the caller was promised")
				}
				if got := l.rows("a", OrgRootPolicy) + l.rows("b", OrgRootPolicy); got != 0 {
					t.Fatalf("the refused mixed batch still wrote %d row(s)", got)
				}
			})
		})
	}

	t.Run("Node has no batch meaning and is refused", func(t *testing.T) {
		a := New(newFakeLedger(), WithTierReader(readerFor(license.TierCommunity, LicenceValid)),
			WithLimits(func(license.Tier) license.TierLimits { return license.TierLimits{MaxNodes: 5} }))
		if _, err := a.AdmitAll(context.Background(), Node, "org", []string{"n1", "n2"}); err == nil {
			t.Fatal("AdmitAll accepted the node dimension. It is a CONCURRENCY question answered by a lease store " +
				"per call; a caller that got a silent loop here would believe it had batch semantics it never had")
		}
	})
}

// fakeRows and memRows expose each ledger's own row count under one name, so
// the table above drives both implementations through identical assertions
// rather than through two nearly-identical bodies.
type fakeRows struct{ *fakeLedger }

func (f fakeRows) rows(org string, d Dimension) int {
	f.mu.Lock()
	defer f.mu.Unlock()
	return f.count(org, d)
}

type memRows struct{ *MemoryLedger }

func (m memRows) rows(org string, d Dimension) int { return m.Rows(org, d) }
...
func TestRuledValuesPerDimension(t *testing.T) {
	want := map[Dimension][3]int{ // community, evaluation, enterprise
		...
		Node:             {1, -1, -1},
		OrgRootPolicy:    {20, 50, -1}, // #3906: was {0, 0, -1}; see tier_limits.go
	}
	for d, w := range want {
...
func TestUnlimitedTierSkipsWithZeroIO(t *testing.T) {
	for _, tier := range []license.Tier{license.TierProfessional, license.TierEnterprise, license.TierEnterprisePlus} {
		...
		// The bound follows the RECORDING dimensions, which is every dimension
		// except Node (#3907 added org_root_policy to the set; see
		// recordUnlimited). Deriving it from Dimensions() rather than writing
		// the number keeps it correct when the set next changes, and the "-1"
		// is Node, which is answered by the lease store and never recorded.
		if maxRecords := 500 * (len(Dimensions()) - 1); ledger.records > maxRecords {
			t.Fatalf("%s: %d record attempts for 500 principals across %d recording dimension(s), cap %d; the seen-set dedupe is not holding",
				tier, ledger.records, len(Dimensions())-1, maxRecords)
		}
...
// TestEveryLifetimeDimensionRecordsOnAnUnlimitedTier pins which dimensions
// recordUnlimited writes, because prose about this has now drifted three times
// and each time in a different direction.
//
// Round 1 wrote a comment saying an Enterprise deployment accumulates
// organization-root rows so they "keep working" after a lapse. Round 2 measured
// 1 / 1 / 0 and the comment was false, so the row was excluded and the test was
// renamed for the two principal dimensions. Round 3 is #3907: the exclusion was
// correct about the MECHANISM - nothing re-admits a policy, so no row makes an
// existing one "keep working" - and wrong about the CONSEQUENCE, because the
// row is what gives AdmitUnderLimit a true count to bound against. It was free
// only while the ceiling was 0, which is what the REVISIT note in
// recordUnlimited said, and #3906 moved that ceiling to 20 and 50.
//
// So the counts are 1 / 1 / 1 / 0 now, and the ONE remaining exclusion is Node,
// which is a concurrency fact answered by node_leases and never a ledger row at
// all. Fixing the sentence a third time is worth less than pinning the numbers,
// which is what this is.
func TestEveryLifetimeDimensionRecordsOnAnUnlimitedTier(t *testing.T) {
	ledger := newFakeLedger()
	...
	// The map must cover the closed set, or a dimension added later is silently
	// unasserted here — which is exactly how this test came to describe two
	// dimensions when there were four.
	for _, d := range Dimensions() {
		if _, ok := want[d]; !ok {
			t.Fatalf("dimension %s has no expected record count; a new dimension must state whether it records on an unlimited tier", d)
		}
	}
	for d, n := range want {
		if got := ledger.count("unl", d); got != n {
			t.Errorf("dimension %s recorded %d row(s) on an unlimited tier, want %d. "+
				...
				"admitOrgRootPolicies both describe the old counts and are now false.", d, got, n)
		}
	}
}
