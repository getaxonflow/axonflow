...
//	                                            path traverses
//	human_principal    admitUserToken           (identity_plane.go) - the HS256
...
// logged once per dimension: a default that is visible on every scrape rather
// than a refusal of every principal during a window the deployment cannot
// avoid.
//
...
// tier limit at one choke point.
func resolveTokenAdmitted(ctx context.Context, orgID, token string) (*sharedidentity.ValidatedIdentity, error) {
	vid, err := sharedidentity.ResolveToken(ctx, orgID, token)
	if err != nil || vid == nil {
		return vid, err
	}
