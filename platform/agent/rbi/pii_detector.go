...
// =============================================================================
// Pre-check Integration Functions
// =============================================================================
...
// blockOnCritical: if true, recommends blocking when critical PII is detected
func CheckRequestForPII(detector *IndiaPIIDetector, query string, blockOnCritical bool) *RBIPIICheckResult {
	...
	criticalPII := GetCriticalFinancialPII(detections)
	hasCritical := len(criticalPII) > 0
	...
	// Recommend blocking if critical PII found and blockOnCritical is set
	if hasCritical && blockOnCritical {
		result.BlockRecommended = true
		result.Reason = buildBlockReason(criticalPII)
	}

	return result
}

// buildBlockReason creates a human-readable reason for blocking
func buildBlockReason(criticalPII []IndiaPIIDetectionResult) string {
	if len(criticalPII) == 0 {
		return ""
	}

	typeNames := make(map[IndiaPIIType]string)
	typeNames[IndiaPIITypeUPI] = "UPI ID"
	typeNames[IndiaPIITypeAadhaar] = "Aadhaar number"
	typeNames[IndiaPIITypePAN] = "PAN card number"
	typeNames[IndiaPIITypeBankAccount] = "Bank account number"

	// Collect unique types
	types := make(map[IndiaPIIType]bool)
	for _, d := range criticalPII {
		types[d.Type] = true
	}
	...
	if len(typeList) == 1 {
		return "Request blocked: Contains " + typeList[0] + " (RBI FREE-AI compliance)"
	}
	return "Request blocked: Contains sensitive financial data (" + strings.Join(typeList, ", ") + ") (RBI FREE-AI compliance)"
}
...
func (d *IndiaPIIDetector) HasCriticalPII(text string) bool {
	detections := d.DetectAll(text)
	critical := GetCriticalFinancialPII(detections)
	return len(critical) > 0
}

// IsEnabled returns whether RBI PII detection is enabled.
// Enterprise build: Always returns true.
func IsEnabled() bool {
	return true
}
