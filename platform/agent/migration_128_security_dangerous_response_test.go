...
func TestMigration128_SecurityDangerousResponsePlane_RealPostgres(t *testing.T) {
	...
	run := func(msg string) OutputPolicyOutcome {
		return evaluateOutputPolicies(ctx, "test-tenant", "u1", "gw.test", nil, msg, nil, 0, false, true /* isGateway */)
	}
	...
	injBlock := evaluateOutputPolicies(ctxBlock, "test-tenant", "u1", "gw.test", nil,
