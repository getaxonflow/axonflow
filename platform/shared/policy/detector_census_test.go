...
// censusNoPostureLever is the only value the posture_lever column may hold.
//
// The column recorded which environment variable (PII_ACTION, SQLI_ACTION,
// SENSITIVE_DATA_ACTION, DANGEROUS_COMMAND_ACTION) replaced a row's stored
// action at runtime - 85 of the 101 rows before v11. #3961 removed the variables
// as action-setters, so no row has one, and the invariant the operator asked for
// is stated as the column being empty rather than as a pinned list of reachable
// rows. The column is kept, with "-" in every row, so that a lever reappearing
// is a diff in this file and a red in TestNoCensusRowCarriesAPostureLever rather
// than a silent absence.
const censusNoPostureLever = "-"
...
// `EvaluatePolicy` was the retired tier engine's
// (platform/agent/tier_aware_policy_engine.go, deleted by #4253), whose
// evaluateFirstMatch did `re.MatchString(input)` and returned - that file
// contained no Validator reference at all. `EvaluateDynamicPolicies` reads dynamic_policies and never
// sees a static row.
//
// THAT WAS TRUE UNTIL #3963 AND IS NOT ANY MORE, and the paragraph is kept
// because the shape of the defect is what this derivation exists to catch.
//
// It read: on the proxy_tier plane every "algorithmic" row runs as a bare
// regex - the Luhn check does not gate sys_pii_credit_card there, and
// sys_pii_booking_ref, validator-inert on the fully-gated planes because its
// category default rejects every non-card string, is NOT inert there and fires
// on any token its pattern matches. proxy_tier was the only plane where NO
// static site ran a validator.
//
// evaluateFirstMatch now resolves the row's validator through
// sharedpolicy.ValidatorFor and runs it against a context window built by
// sharedpolicy.MatchContext, so EvaluatePolicy is validator-bearing and the
// plane is fully gated. The twenty algorithmic rows behave identically on
// every plane that evaluates them.
//
// policy_test WAS the other half of that shape and this comment claimed it
// still was, which was wrong the moment EvaluatePolicy joined the map below.
// policyTestHandler's two static sites are EvaluateRequest and EvaluatePolicy
// (legacy_call_sites.tsv; its third site is EvaluateDynamicPolicies, which
// never reads a static row), so once BOTH are validator-bearing the plane
// computes as planeValidator, not planeMixed. No row renders "(mixed)" any
// more - the suffix appears zero times in detectors_census.tsv.
//
// mixedPlaneSuffix and the planeMixed arm are kept, and are not dead: they are
// the derivation's three-valued answer, and a future plane that gains a bare
// static site must render as mixed rather than silently claiming a gate. What
// changed is that no plane is in that state today.
//
// #4253 DELETED THE TIER ENGINE AND THE proxy_tier PLANE WITH IT, so no call
// site names EvaluatePolicy any more and it has left both sets below. The
// agent's policyTestHandler is no call site either: it previews
// proxy_request's one pass, and policy_test is the orchestrator's dynamic
// plane alone.
...
// one plane, because the legacy plane taxonomy was not consistent about
...
// its EvaluatePolicy one), so each name was unambiguous. policyTestHandler had
// the same two-phase shape and only ONE name, so `policy_test` carried an
// EvaluateRequest site and an EvaluatePolicy site at once. While EvaluatePolicy
// ran no validator that made the plane genuinely half-gated, and labelling it
// validator-bearing would have claimed a gate that half of it did not have.
//
// SINCE #3963 BOTH OF ITS SITES WERE VALIDATOR-BEARING, and since #4253 neither
// handler has a second static phase: /api/request runs one pass, and
// policyTestHandler is no call site. Nothing renders this suffix. The rule
// stands unchanged and is what a future plane with one bare site will land on;
// it simply has no instance right now. Do not "simplify" the derivation to
// binary on the strength of that - binary was the original defect, and nothing
// stops a plane name covering two phases again.
...
func loadValidatorBearingPlanes(t *testing.T) map[string]planeValidatorKind {
	...
	out, err := classifyValidatorPlanes(string(b), validatorBearingEvaluators)
	if err != nil {
		t.Fatalf("%s: %v", callSitesPath, err)
	}
	return out
}

// classifyValidatorPlanes is the three-valued classification itself, with the
// validator-bearing evaluator set as a PARAMETER.
//
// It is separate from the file read because, since #3963, every static
// evaluator is validator-bearing, so over the real table the bare and mixed
// arms have nothing to fire on - and a derivation collapsed to "every plane is
// gated" would regenerate the identical census. The design document names that
// as an unexercised arm. TestTheValidatorPlaneClassifierKeepsAllThreeAnswers
// drives it with a set in which one evaluator is bare.
func classifyValidatorPlanes(callSites string, bearing map[string]bool) (map[string]planeValidatorKind, error) {
	hasValidator := map[string]bool{}
	hasBare := map[string]bool{}
	seenEvaluator := map[string]bool{}
	for i, line := range strings.Split(callSites, "\n") {
		if i == 0 || line == "" {
			continue
		}
		f := strings.Split(line, "\t")
		if len(f) < 2 {
			return nil, fmt.Errorf("line %d: %d field(s), want at least 2", i+1, len(f))
		}
	...
	for name := range staticEvaluators {
		if !seenEvaluator[name] {
			return nil, fmt.Errorf("names no %q call site; the validator classification would be vacuous", name)
		}
	}
	...
	if len(out) == 0 {
		return nil, fmt.Errorf("yields no static plane at all")
	}
	return out, nil
}
...
func TestDetectorCensusClassIsTheCodeThatRuns(t *testing.T) {
	...
	// #3963 follow-up: the exceptions column is the one hand-written field in
	// this census, and after #3963 twenty rows described a defect that no
	// longer existed - "on proxy_tier ... consults no validator, so the bare
	// pattern decides" and "policy_test is MIXED". Both had been true; the
	// same commit that made them false left them in place, because the
	// DERIVED columns regenerate and prose does not. A census that describes a
	// fixed defect as live is worse than one that omits it: the next reader
	// scopes work against it, or files it again.
	//
	// So tie the prose to the derivation rather than to a word list. These two
	// claims are checkable because each has a derived counterpart:
	// plane-dependence is falsified by planes == validator_planes, and a MIXED
	// policy_test is falsified by the plane's own computed kind. A row may
	// still DESCRIBE the history - "was MIXED", "until #3963" - which is why
	// the match is on the present-tense assertion only.
	for _, r := range rows {
		if r.Exceptions == "" || r.Exceptions == "-" {
			continue
		}
		if strings.Contains(r.Exceptions, "policy_test is MIXED") && bearing["policy_test"] != planeMixed {
			t.Errorf("%s: exceptions says \"policy_test is MIXED\" but the derivation computes policy_test as %v. "+
				"Both of policyTestHandler's static sites are validator-bearing since #3963, so nothing renders "+
				"%q. Describe it in the past tense or drop it.", r.PolicyID, bearing["policy_test"], mixedPlaneSuffix)
		}
		if strings.Contains(r.Exceptions, "CLASS IS PLANE-DEPENDENT") && r.Planes == r.ValidatorPlanes {
			t.Errorf("%s: exceptions claims the class is PLANE-DEPENDENT, but planes and validator_planes are "+
				"identical (%q) - every plane that evaluates this row gates it, so there is no plane-dependence "+
				"left to warn about.", r.PolicyID, r.Planes)
		}
	}
	...
	// THE EXPECTED VALUE HERE IS ZERO, AND IT WAS NOT UNTIL #3963.
	//
	// This counts algorithmic rows for which the class is TRUE ONLY ON SOME
	// PLANES - rows whose validator never ran somewhere, leaving the pattern
	// alone to decide. It used to be twenty, and this assertion fired when it
	// reached zero, on the reasoning that "a zero here would mean the
	// qualification had silently stopped being derived, NOT that the problem
	// had gone away". That reasoning was sound while the problem could not go
	// away. `evaluateFirstMatch` now resolves and runs the row's validator, so
	// it has, and the assertion is INVERTED rather than deleted: a row that
	// starts evaluating on a plane whose sites do not validate is #3963
	// happening again.
	t.Logf("algorithmic rows with at least one NON-validator-bearing plane: %d of %d (want 0)",
		algorithmicWithBarePlane, counts[classAlgorithmic])
	if algorithmicWithBarePlane != 0 {
		t.Errorf("%d algorithmic row(s) evaluate on a plane whose static sites run no validator, so "+
			"the shipped detector behaves differently depending on which component evaluated the "+
			"request. That is #3963, closed by wiring the retired tier engine's first-match path to "+
			"sharedpolicy.ValidatorFor.", algorithmicWithBarePlane)
	}

	// AND THE INSTRUMENT MUST STILL BE ABLE TO SAY OTHERWISE.
	//
	// The line above now expects zero, which is the value a BROKEN derivation
	// also produces: if legacy_call_sites.tsv stopped being read, every plane
	// would classify as unknown, censusNonValidatorPlanes would return "-" for
	// every row, and this census would report a perfectly gated platform it had
	// learned nothing about. So the classification is asserted to be live, and
	// to say something specific: /api/request's one pass is gated.
	if len(bearing) == 0 {
		t.Fatal("no plane was classified from legacy_call_sites.tsv; the zero above is the " +
			"derivation failing, not the platform being consistent")
	}
	if bearing["proxy_request"] != planeValidator {
		t.Errorf("proxy_request classifies as %v, want planeValidator. It is /api/request's one pass since #4253 - "+
			"its only static site is EvaluateRequest (proxyDetectorPass), which runs the row's validator - and if it "+
			"has stopped classifying that way the zero above means nothing.", bearing["proxy_request"])
	}
...
// TestNoCensusRowCarriesAPostureLever is the #3961 invariant over the shipped
// corpus: no environment variable can displace any row's stored action, so the
// posture_lever column is "-" on every row. It runs on every tier, including the
// one with no database, over the committed census.
//
// Its population is the whole census (loadDetectorCensus refuses fewer than
// fifty rows), and it counts what it checked, so an emptied or re-keyed census
// cannot pass it by containing nothing. A row carrying any other value - a lever
// name restored by a regenerated census, or a hand edit - is red.
func TestNoCensusRowCarriesAPostureLever(t *testing.T) {
	rows := loadDetectorCensus(t)
	checked := 0
	for _, r := range rows {
		checked++
		if r.PostureLever != censusNoPostureLever {
			t.Errorf("%s (category %s) carries posture_lever %q: no environment variable sets an action any more (#3961), "+
				"so no row may name one", r.PolicyID, r.Category, r.PostureLever)
		}
	}
	if checked != len(rows) || checked < 50 {
		t.Fatalf("checked %d of %d census rows; the invariant is over the whole shipped corpus", checked, len(rows))
	}
	t.Logf("posture_lever is %q on all %d census rows", censusNoPostureLever, checked)
}
...
// different planes. TWELVE of the hundred-and-one rows do. Eight carry
// retired_tier_pass_action: the four sys_admin_* rows and the template's four
// block rows deny on proxy_request, where /api/request's retired second pass
// read their stored block, and signal on the planes that resolve the phase
// columns. The other four, the sys_dangerous_injection_* rows, carry
// redact_target_not_stored. Collapsing the twelve to the strongest value would
// hide a per-plane difference behind a column that looked decided. Until #4253
// the divergent rows were the read_path_action_divergence class, the corpus's
// largest; its diverging read path was the retired proxy_tier plane's, and no
// row carries that reason now.
...
func derivedCensusRow(raw legacycompile.RawRow, rec legacycompile.Record, seededBy map[string]string, bearing map[string]planeValidatorKind) detectorCensusRow {
	...
	return detectorCensusRow{
		...
		PostureLever:       censusNoPostureLever,
		Disposition:        status,
...
func TestSupersessionLedgerIsCompleteAndHonest(t *testing.T) {
	...
	superseded := 0
	for _, f := range recs {
		// f[5] is the recorded decision; its precondition is checked with no
		// database by registry.CheckSupersessionLedgerAgainstCensus and
		// legacycompile.CheckSupersessionDecisions, so it is not restated here.
		id, seededBy, supersededBy, stillLive, tracking, note := f[0], f[1], f[2], f[3], f[4], f[6]
		declared[id] = true
	...
	for _, f := range recs {
		...
		for _, sup := range strings.Split(supersededBy, ",") {
			...
			// the retired decision shadow mirrored it. What
