...
func TestNewDynamicPolicyEvaluatorFromEnv(t *testing.T) {
	...
	origEndpoint := os.Getenv("MCP_DYNAMIC_POLICIES_ENDPOINT")
	...
	defer func() {
		...
		restoreEnv("MCP_DYNAMIC_POLICIES_ENDPOINT", origEndpoint)
	...
	os.Setenv("MCP_DYNAMIC_POLICIES_ENDPOINT", "http://custom:9000")
	...
	if config.OrchestratorEndpoint != "http://custom:9000" {
		t.Errorf("expected endpoint=http://custom:9000, got %s", config.OrchestratorEndpoint)
	}
	if config.Timeout != 10*time.Second {
		t.Errorf("expected Timeout=10s, got %v", config.Timeout)
	}
...
func TestInitGlobalDynamicPolicyEvaluator(t *testing.T) {
	...
	defer restoreEnv("MCP_DYNAMIC_POLICIES_ENDPOINT", origEndpoint)
	defer restoreEnv("MCP_DYNAMIC_POLICIES_TIMEOUT", origTimeout)
	defer restoreEnv("MCP_DYNAMIC_POLICIES_GRACEFUL", origGraceful)

	os.Setenv("MCP_DYNAMIC_POLICIES_ENABLED", "true")
	os.Setenv("MCP_DYNAMIC_POLICIES_ENDPOINT", "http://init-test:9000")
	...
	if config.OrchestratorEndpoint != "http://init-test:9000" {
		t.Errorf("expected endpoint=http://init-test:9000, got %s", config.OrchestratorEndpoint)
	}
