...
//     token without one is admitted, and a token minted for a DIFFERENT
//     audience is refused. Neither production minter sets `aud`, so this too
//     is latent.
//
// Everything else is derived from the legacy path.
//
// That is the difference between realms that admit what the platform already
// verified and realms that refuse it. If these realms declared an audience the
// platform never checked, or an assurance floor nobody ever attested, every
// single request would be refused for a reason of this file's own invention. So: the minimum assurance is the
...
// was removed rather than kept unused: TrustedHeaderPrincipal stamps
...
func NewBuiltinRealmSource(registry *RealmRegistry, dep BuiltinRealmDeployment, extra ...RealmSource) (RealmSource, error) {
	if registry == nil {
