...
func TestExportRepository_GetByID_Success(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`FROM euaiact_exports WHERE id = $1`)).
		WithArgs("exp-001").
		WillReturnRows(rows)

	export, err := repo.GetByID(ctx, "exp-001")
	require.NoError(t, err)
...
func TestExportRepository_GetByID_NotFound(t *testing.T) {
	...
	export, err := repo.GetByID(ctx, "nonexistent")
	assert.NoError(t, err)
	assert.Nil(t, export)
	assert.NoError(t, mock.ExpectationsWereMet())
}
...
func TestExportRepository_Delete_Success(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta(`DELETE FROM euaiact_exports WHERE id = $1`)).
		WithArgs("exp-001").
		WillReturnResult(sqlmock.NewResult(0, 1))

	err = repo.Delete(ctx, "exp-001")
	assert.NoError(t, err)
	assert.NoError(t, mock.ExpectationsWereMet())
}
...
func TestExportRepository_GetByID_NullOptionalFields(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`FROM euaiact_exports WHERE id = $1`)).
		WithArgs("exp-minimal").
		WillReturnRows(rows)

	export, err := repo.GetByID(ctx, "exp-minimal")
	require.NoError(t, err)
