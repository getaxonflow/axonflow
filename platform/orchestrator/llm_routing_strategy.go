...
// RoutingConfig holds the routing configuration loaded from environment
...
func LoadRoutingConfig() RoutingConfig {
	config := RoutingConfig{
		Strategy:        RoutingStrategyWeighted, // Default
...
//   - loadBalancer: used for weighted random selection
//
// Returns the selected provider name
func (s *ProviderSelector) SelectProvider(
	...
	case RoutingStrategyWeighted:
		fallthrough
	default:
		return s.selectWeighted(healthyProviders, weights, loadBalancer)
	}
}

// selectWeighted performs weighted random selection
func (s *ProviderSelector) selectWeighted(
	healthyProviders []string,
	weights map[string]float64,
	loadBalancer *LoadBalancer,
) string {
	return loadBalancer.SelectProvider(healthyProviders, weights)
}
...
func (s *ProviderSelector) selectFailover(
	healthyProviders []string,
	weights map[string]float64,
) string {
