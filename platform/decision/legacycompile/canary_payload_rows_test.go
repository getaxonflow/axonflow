...
// seedInserts reads the canary's seed migrations through the package's one
// extraction (seed_sql.go, where the column-order and semicolon traps it
// guards against are recorded), and refuses to check nothing.
func seedInserts(t *testing.T) []insertStmt {
	t.Helper()
	out, err := readSeedInserts(canarySeedMigrations)
	if err != nil {
		t.Fatalf("%v.\n\nThe seed migrations moved; this test addresses them by "+
			"path and would otherwise silently check nothing.", err)
	}
	return out
}
