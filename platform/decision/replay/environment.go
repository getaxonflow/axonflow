...
func (e *Environment) Engine(ctx context.Context) (*pdp.Engine, error) {
	...
	engine, err := pdp.NewEngine(ctx, pdp.EngineConfig{
		...
		Documents:  docs,
		TrustStore: ts,
		// UNANCHORED, with the reason. A replay reconstructs the environment a
		// RECORDED decision was taken in, which is by definition the corpus
		// that deployment had at that time rather than the one this binary
		// ships. Anchoring here would make a replay of any earlier release
		// refuse, and a replay that cannot reproduce history is not a replay.
		SystemCorpus:  pdp.Unanchored("a replay reconstructs the recorded environment of a past decision, whose system document is the one that deployment had rather than the one this binary ships"),
		Registry:      e.Registry,
		PEP:           e.PEP,
