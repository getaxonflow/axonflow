...
- **Verification endpoints** (`platform/agent/audit_verification_handler.go`):
  ...
  - **All three require compliance read authority over the organization** (#2914), not merely authentication: a validated per-user token whose role is `admin`, `owner` or `policy_admin`, or an internal service asserting `X-Axonflow-Admin-Authority: true`. Community mode is exempt, having no authentication to hold a role. The refusal is `403`. Before this, any authenticated member of an organization could verify - and so enumerate - its records by id, which put each record's `decision_type`, `risk_level` and chain linkage in reach of roles that hold no tenant-wide audit read.
