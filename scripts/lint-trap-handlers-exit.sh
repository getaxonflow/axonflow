...
case \"\$1\" in
  ...
  #     reaches the three known-correct sites and really classifies them.
  ran=$((ran + 1))
  local seen=0
  for probe in scripts/ssm-tunnel-rds.sh scripts/ssm-tunnel-invpc.sh scripts/seed-portal-playwright.sh; do
    if LC_ALL=C grep -qE '^[[:space:]]*trap[[:space:]]' "$REPO_ROOT/$probe" 2>/dev/null; then
      seen=$((seen + 1))
    fi
  done
  # And that the walk itself finds shell files at all.
  local nfiles
  nfiles=$(find "$REPO_ROOT/scripts" -maxdepth 1 -name '*.sh' -type f | wc -l | tr -d ' ')
  if [ "$seen" -eq 3 ] && [ "$nfiles" -gt 20 ]; then
    echo "  ok   the real scan reaches the three known-correct sites ($nfiles scripts/ shell files)"
  else
    echo "  FAIL anti-vacuity: seen=$seen/3 known-correct sites, $nfiles shell files under scripts/"
    fails=$((fails + 1))
  fi
