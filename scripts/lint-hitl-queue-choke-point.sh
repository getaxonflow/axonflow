...
run_lint() {
  ...
  # axonflow#483): this file syncs to the community mirror and must prove
  # itself on the MIRROR'S OWN INPUTS. The allow-list names one file under
  # scripts/e2e/, which the sync strips, so on the mirror the scan found three
  # of four and the guard reported a violation where nothing was written. On
  # the community tree an allow-listed file the sync excluded is reported and
  # dropped from the expectation; on the enterprise tree, and in every
  # fixture, an absent allow-listed file stays what it always was - the choke
  # point disappearing - and fails. Nothing is lowered where the file exists.
  if is_community_tree "$root"; then
    local kept="" line path
    while IFS= read -r line; do
      ...
      if [ -f "$root/$path" ]; then
        ...
        echo "   .. community tree: allow-list entry absent by the sync's own exclusions, not expected here: $path"
      fi
    done <<< "$want"
    want=$(printf '%s' "$kept")
    if [ -z "$want" ]; then
      echo "❌ HITL queue choke-point lint FAILED: no allow-listed writer is present on this community tree; the choke point itself is missing."
      return 1
    fi
  fi
  got=$(scan "$root")
...
z := 3 // delete from decisions, hitl_history, hitl_approval_queue in that order"
  ...
  # EDITION MARKER, both directions, on fixtures (axonflow#483). A community
  # tree (lint.yml present, sync workflow absent) tolerates an allow-listed
  # file the sync excluded; the same tree with the sync workflow present is
  # the enterprise shape and the absence is the choke point disappearing; and
  # the tolerance never admits a writer the allow-list does not name.
  local mir="$tmp/mir"
  mkok "$mir"
  rm -f "$mir/scripts/e2e/fixtures/readiness-gate-shape/decoy/05-curl-fail-flag.sh"
  mk "$mir/.github/workflows/lint.yml" "name: Lint"
  check "community tree: an allow-listed file the sync excludes is reported, not a violation" 0 "$mir"
  local mirstrict="$tmp/mirstrict"
  cp -R "$mir" "$mirstrict"
  mk "$mirstrict/.github/workflows/sync-community-repo.yml" "name: Sync"
  check "enterprise tree: the same absence is the choke point disappearing" 1 "$mirstrict"
  local mirstray="$tmp/mirstray"
  cp -R "$mir" "$mirstray"
  mk "$mirstray/platform/orchestrator/stray.go" "const q = \`$NEEDLE (a) VALUES (\$1)\`"
  check "community tree: a writer outside the allow-list still fails" 1 "$mirstray"
